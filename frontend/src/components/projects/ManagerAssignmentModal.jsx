...
export default function ManagerAssignmentModal({ project, onClose }) {
  const { assignManager, loading: storeLoading } = useProjectStore();

  const [employees, setEmployees] = useState([]);
  const [selectedId, setSelectedId] = useState(project.managerId || '');
  ...
  const handleSave = async (e) => {
    ...
    const result = await assignManager(project.id, selectedId || null);
    if (result.success) {
      onClose();
    } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 select-none">
      <div className="glass w-full max-w-md rounded-2xl border border-slateDark-800 overflow-hidden shadow-2xl animate-scale-up">
        ...
        <div className="px-6 py-4 border-b border-slateDark-900 flex justify-between items-center">
          <h3 className="font-extrabold text-white text-lg">Assign Project Manager</h3>
          <button
            onClick={onClose}
